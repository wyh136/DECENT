...
bool CWalletDB::WritePass(uint256 hash, uint256 pass)
{
    nWalletDBUpdated++;
    return Write(make_pair(string("pass"), hash), pass);
}

bool CWalletDB::ErasePass(uint256 hash)
{
    nWalletDBUpdated++;
    return Erase(make_pair(string("pass"), hash));
}

bool CWalletDB::WriteElGamalKey(valtype key)
{
    nWalletDBUpdated++;
    return Write(make_pair(string("ElGamal"), string("key")), key);
}
...
int CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    //// todo: shouldn't we catch exceptions and try to recover and continue?
    {
        ...
        loop
        {
            ...
            else if (strType == "pass")
            {
                uint256 hash;
                ssKey >> hash;
                ssValue >> pwallet->mapPassword[hash];
            }
            else if (strType == "ElGamal")
            {
                ssValue >> pwallet->privateElGamalKey;
            }
            else if (strType == "tx")
            {
                uint256 hash;
