...
boost::filesystem::path GetConfigFile()
{
    ...
    fs::path pathConfigFile(GetArg("-conf", "decent.conf"));
...
boost::filesystem::path GetPidFile()
{
    ...
    fs::path pathPidFile(GetArg("-pid", "decent.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong Decent will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("Decent"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
{
    return GetAutostartDir() / "decent.desktop";
}
