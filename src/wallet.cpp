...
bool CWallet::SelectCoins(int64 nTargetValue, unsigned int nSpendTime, list<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
    ...
    {
        if (nValue < 0)
            return error("create tx : nValue < 0");
        nValue += s.second;
    }
    if (vecSend.empty() || nValue < 0)
        return error("create tx : vecSend.empty()");
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn))
                    return error("create tx : SelectCoins() failed");
...
bool CWallet::CreateCoinStake(unsigned int nBits, int64 nSearchInterval, CTransaction& txNew, int64 rewardForBlockCreator)
{
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake))
            {
                ...
                if (whichType == TX_PUBKEYHASH) // pay to address type
                {
                    ...
                    CKey key;
                    if (!this->GetKey(uint160(vSolutions[0]), key))
                    {
                        if (fDebug && GetBoolArg("-printcoinstake"))
                ...
                txNew.vout.push_back(CTxOut(0, scriptPubKeyOut)); // Decent: for reward
...
//    }
    ...
    loop
    {
        ...
        {
            txNew.vout[1].nValue = rewardForBlockCreator; // Decent: set reward for block creator
            txNew.vout[2].nValue = ((nCredit - nMinFee) / 2 / CENT) * CENT;
            txNew.vout[3].nValue = nCredit - nMinFee - txNew.vout[2].nValue;
        }
        else
            txNew.vout[2].nValue = nCredit - nMinFee;
        //You can put your own extra vouts inside Stake transaction in such way, but it will have
        //consequences.First, you shouldn't hardcode any determined address here, because we have
        //seen strange thing.All stake money, used for coinstake transaction, will be returned to
        //this determined address regardless of person, who minted it.But money, which we sent
        //to this address, were delivered to address of person, who minted it.We don't know, why it
        //was happend, but it looked like this outputs exchanged their delivery addresses.
        //In conclusion, you should find out with coinstake transaction building process or you
        //should never ever touch this process.
//        CScript Script;
//        CBitcoinAddress address = CBitcoinAddress("SU6AncdVHCpDAiG9yj9BdNscb83XeLTpcj");
//        Script.SetBitcoinAddress(address);
//        txNew.vout.push_back(CTxOut(100000000, Script));
...
bool CWallet::CommitDecentTX(CWalletTx& wtxNew, CReserveKey& reservekey)
{
    printf("Commit Decent tx:\n%s", wtxNew.ToString().c_str());
    mapRequestCount[wtxNew.GetHash()] = 0;

    if (!wtxNew.AcceptToMemoryPool())
    {
        if (wtxNew.IsRedundant())
            return true;
        printf("CommitTransaction() : Error: Transaction not valid");
        return error("Decent tx is not valid");
    }

    AddToWalletIfInvolvingMe(wtxNew, NULL, true, true);
    wtxNew.RelayWalletTransaction();
    MainFrameRepaint();
    return true;
}
...
bool CWallet::AddPassword(const uint256 &hash, const uint256 &password)
{
    mapPassword[hash] = password;
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WritePass(hash, password);
}

bool CWallet::DelPassword(const uint256 &hash)
{
    mapPassword.erase(hash);
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).ErasePass(hash);
}

bool CWallet::GetPassword(const uint256 &hash, uint256 &password)
{
    if (mapPassword.count(hash))
    {
        password = mapPassword[hash];
        return true;
    }
    return false;
}

valtype CWallet::GetElGamalPublicKey()
{
    return EncryptionUtils::GetPublicElGamalKey(GetElGamalPrivateKey());
}

valtype CWallet::GetElGamalPrivateKey()
{
    valtype retKey;
    if (!privateElGamalKey.size())
    {
        // Generate new key
        retKey = EncryptionUtils::GeneratePrivateElGamalKey();
        {
            LOCK(cs_wallet);
            privateElGamalKey = retKey;
            if (!fFileBacked)
                assert(false);
            if (!CWalletDB(strWalletFile).WriteElGamalKey(retKey))
                assert(false);
        }
    }
    else
